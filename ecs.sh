...
aws ecs register-task-definition --family ${TASK_FAMILY} --network-mode awsvpc --requires-compatibilities FARGATE --execution-role-arn arn:aws:iam::${aws_account}:role/ecsTaskExecutionRole --cpu 256 --memory 512 --container-definitions "name=${SERVICE_NAME},image=$aws_account.dkr.ecr.$aws_region.amazonaws.com/learn:apache_v_${BUILD_NUMBER},portMappings=[{containerPort=80}]"
...
aws ecs update-service --cluster "PRS-DEV-CLUSTER" --service "${SERVICE_NAME}" --task-definition "${TASK_FAMILY}":"${BUILD_NUMBER}" --desired-count 1 --force-new-deployment
